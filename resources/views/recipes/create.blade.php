    ...
    <form action="{{ route('recipe.store') }}" method="POST" class="w-10/12 p-4 mx-auto bg-white rounded" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-2 rounded border border-gray-500 my-4">
            ...
            @foreach($categories as $c)
                <option value="{{ $c['id'] }}">{{ $c['name'] }}</opyion>
            @endforeach
                </select>
                <div class="flex justify-end">
                    <button type="submit" class="bg-green-500 hover:bg-green-700 text-white font-bold 
                    py-2 px-4 rounded">レシピを投稿する</button>
                </div>
            </div>
        </div>
        <hr class="my-4">
        ...
        <button type ="button" id="step-add" class="bg-gray-500 hover:bg-gray-700 text-white
        font-bold py-2 px-4 rounded">手順を追加する</button>
    </form>
</x-app-layout>